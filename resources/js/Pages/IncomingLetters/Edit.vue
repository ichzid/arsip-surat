...
const form = useForm({
    ...
    received_date: props.letter.received_date ? props.letter.received_date.split('T')[0] : '',
    origin: props.letter.origin,
    subject: props.letter.subject,
    file: null,
});

const submit = () => {
    form.post(route('incoming-letters.update', props.letter.id));
};
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
                Edit Surat Masuk: {{ letter.agenda_number }}
            </h2>
        </template>
                    ...
                    <form @submit.prevent="submit">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input v-model="form.received_date" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            </div>
